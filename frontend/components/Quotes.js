...
import { useGetQuotesQuery, useSetFakeMutation, useDeleteQuoteMutation } from '../state/quotesApi'

export default function Quotes() {
  const { data: quotes, isLoading: gettingQuotes, isFetching: refreshingQuotes } = useGetQuotesQuery()
  const [setFake, { isLoading: togglingQuote }] = useSetFakeMutation()
            ...
            .map(qt => (
              ...
              >
                ...
                <div className="quote-buttons">
                  ...
                  <button onClick={() => setFake({id: qt.id, quote: { apocryphal: !qt.apocryphal}})}>FAKE</button>
                </div>
              </div>
            ))
        }
